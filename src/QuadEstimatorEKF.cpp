...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
	...
	V3F accel_inertial_frame = attitude.Rotate_BtoI(accel);

	predictedState(0) = curState(0) + dt * curState(3);
	predictedState(1) = curState(1) + dt * curState(4);
	predictedState(2) = curState(2) + dt * curState(5);
	predictedState(3) = curState(3) + dt * accel_inertial_frame.x;
	predictedState(4) = curState(4) + dt * accel_inertial_frame.y;
	predictedState(5) = curState(5) + dt * accel_inertial_frame.z - dt * CONST_GRAVITY;
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
	...
	////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
	// Precalculated Roll angle phi variables: cos(phi), sin(phi)
	float cos_phi = cos(roll);
	float sin_phi = sin(roll);

	// Precalculated Pitch angle theta variables: cos(theta), sin(theta)
	float cos_theta = cos(pitch);
	float sin_theta = sin(pitch);


	// Precalculated Yaw angle psi variables: cos(psi), sin(psi)
